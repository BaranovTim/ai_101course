"""Seed the General track with the AI 101 course: modules, lessons, quizzes.

Usage: python manage.py seed_course
Idempotent — re-running replaces the course content (learner progress on
...
from courses.models import Choice, Course, Lesson, Module, Question, Quiz, Track
...
class Command(BaseCommand):
    help = "Seed the General track and the AI 101 course content."

    @transaction.atomic
    def handle(self, *args, **options):
        track, t_created = Track.objects.update_or_create(
            slug=TRACK_SLUG,
            defaults={
                "name": "General AI Track",
                "description": "The foundation for everyone: AI fundamentals, prompting, and practical daily workflows.",
                "icon": "school",
                "audience": "general",
                "price_cents": 10000,
        ...
        self.stdout.write(("Created" if t_created else "Updated") + f" track: {track.name}")
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Seeded '{track.name}' → {course.modules.count()} modules, "
                f"{total_lessons} lessons, {total_questions} quiz questions."
            )
        )
